...
`validate(&schema)`, `label_names(&schema)`. `Series` is one row, built from
plain values with `Series::new(labels, timestamps, values)`, which checks the
lengths, the sample order and duplicate names once. `encode(names,
&[Series])` puts whole-series rows into a batch and refuses two with one
label set, so a store that sends a series as several chunk rows builds the
batch in Arrow and needs only `validate`, the way `memory.rs` slices an
encoded batch into chunk rows. `decode(&[RecordBatch])` slices rows back out
without copying, one `Series` per chunk row. A store that already has Arrow
(Parquet, Vortex) builds the batch directly and only needs `validate`; a
store that has whole series as rows builds `Series` and calls `encode`.

## Implementing it

**In memory** (`memory.rs`). The stored series are encoded once and sorted
by label set. Filter them with `matcher.rs`, clip each to the range with two
binary searches and a slice, cut it into chunk rows that slice the same
samples buffer, return DataFusion's memory source; `SeriesSetExec` declares
and checks the order above it. One row per series by default; the tests also
