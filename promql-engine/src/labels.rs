...
//! but it marks every field nullable and forgets dictionary encoding, so
//! the result would no longer be the canonical shape and the next
//! operator's schema check would reject it. This function is
//! `named_struct` with the canonical field types promised up front.
//!
//! Values arrive either as `Dictionary<UInt32, Utf8>` (read straight out
//! of an existing struct with `get_field`) or as `Utf8` (a grouping key,
//! cast so that DataFusion's hash aggregation stays on its fast path).
//! Both come out as the canonical dictionary leaf. NULLs, should any
//! appear, become `""`, which is how the shape spells "absent".
//!
...
use datafusion::arrow::array::{Array, ArrayRef, AsArray, StringDictionaryBuilder, StructArray};
use datafusion::arrow::compute::cast;
use datafusion::arrow::datatypes::{DataType, Field, FieldRef, Fields, UInt32Type};
use datafusion::common::{plan_err, ScalarValue};
use datafusion::error::{DataFusionError, Result};
use datafusion::functions::core::expr_fn::get_field;
use datafusion::logical_expr::{
    cast as cast_expr, col, lit, ColumnarValue, Expr, ReturnFieldArgs, ScalarFunctionArgs,
...
/// One value column as the canonical dictionary leaf, NULLs as `""`.
fn canonical(values: &ArrayRef) -> Result<ArrayRef> {
    if values.data_type() == &series::label_type() && values.null_count() == 0 {
        return Ok(Arc::clone(values));
    }
    let strings = cast(values.as_ref(), &DataType::Utf8)?;
    let strings = strings.as_string::<i32>();
    let mut b = StringDictionaryBuilder::<UInt32Type>::with_capacity(strings.len(), 16, 256);
    for v in strings {
...
/// `CAST(get_field(labels, k) AS Utf8) AS k`. Plain strings, because
/// DataFusion's vectorized group-by does not take dictionaries.
pub fn group_exprs(keys: &[String]) -> Vec<Expr> {
    keys.iter()
        .map(|k| cast_expr(get_field(col(LABELS), k.as_str()), DataType::Utf8).alias(k))
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn canonical_encodes_strings_and_fills_nulls() {
