...
pub fn fold(expr: &Expr, ts_ms: i64) -> Result<f64, EngineError> {
    match expr {
        ...
        Expr::Call(c) => match c.func.name.as_str() {
...
mod tests {
    ...
    fn the_literal_corpus_folds_to_its_expected_values() {
        ...
        // `2.` is in the corpus too, but our lexer rejects a trailing
        // dot that Prometheus's `scanNumber` accepts. A parser gap, not
        // this fold's: the expression never gets here.
