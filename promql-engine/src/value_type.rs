...
/// The seven functions in `promql/parser/functions.go` at 83962c35 whose
/// `ReturnType` is `ValueTypeScalar`. The other eighty return a vector
/// and none returns a string or a matrix, so listing the exceptions is
/// the whole table — and an unknown name defaulting to `Vector` then
/// agrees with upstream for every function it could be.
///
/// `start`, `end`, `range` and `step` are the experimental duration
/// functions; they are here because the corpus parses with experimental
/// functions enabled.
const SCALAR_FUNCTIONS: [&str; 7] = ["end", "pi", "range", "scalar", "start", "step", "time"];
