...
// A load block inside a `.test` script is the same format the shared
// YAML corpus uses, so it is parsed by the same code. Re-exported
// because these types appear in this module's own public API.
pub use promql_testcases::{LoadBlock, SeriesLine, Unsupported, UnsupportedLine};
...
pub enum Error {
    ...
    #[error("load block: {0}")]
    Load(#[from] promql_testcases::Error),
}
...
fn parse_load(file: &str, lines: &[&str], i: usize) -> Result<(usize, Load), Error> {
    ...
    // Hand the whole block to the shared parser: the `load <interval>`
    // directive and its series lines are the same format the YAML
    // corpus uses, matched the same way upstream matches it.
    let raw = lines[i..end].join("\n");
    let block = promql_testcases::parse_load_block(&format!("{file}:{}", i + 1), &raw)?;
